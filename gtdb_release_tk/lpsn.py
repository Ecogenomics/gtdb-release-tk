...
class LPSN(object):
    ...
    def __init__(self, lpsn_data, lpsn_gss_metadata_file):
        ...
        if lpsn_gss_metadata_file:
            self.parse_lpsn_gss_file(lpsn_gss_metadata_file)
    ...
    def parse_lpsn_gss_file(self, lpsn_gss_metadata_file):
        ...
        self.species = set()
        self.sp_correct_names = {}
        self.sp_synonyms = defaultdict(set)

        self.genera = set()
        self.genus_correct_names = {}
        self.genus_synonyms = defaultdict(set)
        with open(lpsn_gss_metadata_file, encoding='utf-8') as f:
            csv_reader = csv.reader(f)
            ...
            for line_num, tokens in enumerate(csv_reader):
                if line_num == 0:
                    ...
                    if tokens[genus_idx] != '' and tokens[sp_idx] != '' and tokens[subsp_idx] != '':
                        ...
                        # process species
                        taxon = 's__{} {}'.format(tokens[genus_idx].strip(), tokens[sp_idx].strip())
                        self.species.add(taxon)

                        if 'correct name' in tokens[status_idx]:
                            assert tokens[record_lnk_idx] == ''
                            self.sp_correct_names[taxon] = taxon
                        elif tokens[record_lnk_idx] != '':
                            # should link to the correct name for taxon according to LPSN
                            self.sp_correct_names[taxon] = lpsn_record_taxon_map[tokens[record_lnk_idx]]

                        if 'synonym' in tokens[status_idx] and tokens[record_lnk_idx] != '':
                            synonym_sp = lpsn_record_taxon_map[tokens[record_lnk_idx]]
                            self.sp_synonyms[taxon].add(synonym_sp)
                    else:
                        # process genus
                        taxon = f'g__{tokens[genus_idx].strip()}'
                        self.genera.add(taxon)

                        if 'correct name' in tokens[status_idx]:
                            assert tokens[record_lnk_idx] == ''
                            self.genus_correct_names[taxon] = taxon
                        elif tokens[record_lnk_idx] != '':
                            # should link to the correct name for taxon according to LPSN
                            self.genus_correct_names[taxon] = lpsn_record_taxon_map[tokens[record_lnk_idx]]

                        if 'synonym' in tokens[status_idx] and tokens[record_lnk_idx] != '':
                            synonym_genus = lpsn_record_taxon_map[tokens[record_lnk_idx]]
                            self.genus_synonyms[taxon].add(synonym_genus)
